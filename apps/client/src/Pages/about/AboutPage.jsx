  ...
  return (
    <main className="bg-white pt-28 text-slate-950">
      <section className="mx-auto grid min-h-[calc(100vh-96px)] max-w-7xl items-center gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[1fr_0.9fr] lg:px-10">
        <div>
          <span
            className={`rounded-full ${theme.bgSoft} px-4 py-1.5 text-sm font-black uppercase tracking-widest ${theme.text}`}>
            About {data.profile.brand}
          </span>
          <h1 className="mt-6 max-w-4xl text-4xl font-black leading-tight sm:text-5xl">
            {data.profile.aboutHeadline ||
              "Developer-led delivery for businesses that need practical software."}
          </h1>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <button
              className={`rounded-2xl ${theme.bg} px-6 py-3 font-black text-white ${theme.bgHover}`}
              onClick={() => navigate("/projects")}>
              View work
            </button>
            <button
              className={`rounded-2xl border border-slate-300 px-6 py-3 font-black text-slate-950 ${theme.borderHover} ${theme.textHover}`}
